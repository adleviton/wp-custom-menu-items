...
class custom_menu_items
{
  ...
  private function register()
  {
    if (!is_admin() && !$this->has_registered) {
      ...
      add_filter('wp_get_nav_menu_items', [$this, 'wp_get_nav_menu_items'], 20, 2);
      add_filter('wp_get_nav_menu_object', [$this, 'wp_get_nav_menu_object'], 20, 2);
    }
  }
   ...
   * @param null $ID
   */
  public static function add_item($menu_slug, $title, $url, $order = 0, $parent = 0, $ID = null, $classes = [])
  {
    ...
    if (rtrim($current_url, '/') === rtrim($url, '/') || (!empty($parts['path']) && $parts['path'] === $url)) {
   ...
   * @param null $ID
   */
  public static function add_object($menu_slug, $object_ID, $object_type = 'post', $order = 0, $parent = 0, $ID = null, $classes = [])
  {
    $instance = custom_menu_items::get_instance();
    $instance->menus[$menu_slug] = $menu_slug;

    if ('post' == $object_type && $object = get_post($object_ID)) {
      $instance->menu_items[] = [
        'menu' => $menu_slug,
        'order' => $order,
        'parent' => $parent,
